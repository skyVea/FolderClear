...
public final class StringUtils {
	 ...
	 * @return 根据路径获取文件名
	 */
	public static String appendStringForPath(String pathname, String datestring) {
		if (!isEmpty(pathname)) {
			int suffixindex = pathname.lastIndexOf(".");
			return pathname.substring(0, suffixindex).concat("_").concat(datestring)
					.concat(pathname.substring(suffixindex, pathname.length()));
		} else {
			return pathname;
		}
	}
